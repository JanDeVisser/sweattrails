CC = clang
CFLAGS = -Wall -Wextra -O2 $(shell pkg-config --cflags raylib libcurl openssl)

UNAME_S := $(shell uname -s)
ifeq ($(UNAME_S),Darwin)
    LDFLAGS = $(shell pkg-config --libs raylib libcurl openssl) -framework IOKit -framework Cocoa -framework OpenGL
else
    LDFLAGS = $(shell pkg-config --libs raylib libcurl openssl) -lm
endif

TARGET = sweattrails
SRCS = main.c fit_parser.c strava_api.c wahoo_api.c file_organizer.c activity_tree.c tile_map.c zwift_worlds.c activity_meta.c zwift_sync.c garmin_sync.c
OBJS = $(SRCS:.c=.o)

all: $(TARGET)

$(TARGET): $(OBJS)
	$(CC) -o $@ $^ $(LDFLAGS)

%.o: %.c
	$(CC) $(CFLAGS) -c -o $@ $<

clean:
	rm -f $(TARGET) $(OBJS)

run: $(TARGET)
	./$(TARGET)

.PHONY: all clean run
