...
srcfile = srtm.data.__file__
package_location = srcfile[: srcfile.rfind(os.path.sep)]
DEFAULT_LIST_JSON = os.path.join(package_location, 'list.json')
...
def get_data(srtm1=True, srtm3=True, leave_zipped=False, file_handler=None,
    ...
    return srtm.data.GeoElevationData(srtm1_files, srtm3_files, file_handler=file_handler,
...
def _get_urls(use_included_urls, file_handler):
    ...
    try:
        ...
        srtm1_files = srtm.retriever.retrieve_all_files_urls(SRTM1_URL)
        srtm3_files = srtm.retriever.retrieve_all_files_urls(SRTM3_URL)
...
class FileHandler:
    ...
    def exists(self, file_name):
        return os.path.exists(os.path.join(self.get_srtm_dir(), file_name))

    def write(self, file_name, contents):
        with open(os.path.join(self.get_srtm_dir(), file_name), 'wb') as f:
            f.write(contents)

    def read(self, file_name):
        with open(os.path.join(self.get_srtm_dir(), file_name), 'rb') as f:
            return f.read()
