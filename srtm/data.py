...
class GeoElevationData:
    ...
    def __init__(self, srtm1_files, srtm3_files, leave_zipped=False,
        ...
        self.file_handler = file_handler if file_handler else srtm.utils.FileHandler()
    ...
    def get_elevation(self, latitude, longitude, approximate=None):
        ...
        #logger.debug('File for ({0}, {1}) -> {2}'.format(
        ...
        for row in range(height):
            for column in range(width):
                ...
                if elevation == None:
                    ...
                    color = srtm.utils.get_color_between(min_color, max_color, elevation_coef)
                    if elevation <= 0:
                        color = zero_color
...
class GeoElevationFile:
    ...
    def approximation(self, latitude, longitude):
        ...
        d_meters = d * srtm.utils.ONE_DEGREE
        ...
        # here:
        importance_1 = d_meters - srtm.utils.distance(latitude + d, longitude, latitude, longitude)
        elevation_1  = self.geo_elevation_data.get_elevation(latitude + d, longitude, approximate=False)

        importance_2 = d_meters - srtm.utils.distance(latitude - d, longitude, latitude, longitude)
        elevation_2  = self.geo_elevation_data.get_elevation(latitude - d, longitude, approximate=False)

        importance_3 = d_meters - srtm.utils.distance(latitude, longitude + d, latitude, longitude)
        elevation_3  = self.geo_elevation_data.get_elevation(latitude, longitude + d, approximate=False)

        importance_4 = d_meters - srtm.utils.distance(latitude, longitude - d, latitude, longitude)
